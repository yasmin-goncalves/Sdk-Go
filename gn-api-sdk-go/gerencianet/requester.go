...
func newRequester(clientID string, clientSecret string, sandbox bool, timeout int) *requester {
	auth := newAuth(clientID, clientSecret, sandbox, timeout)
	httpClient := &http.Client{Timeout: time.Second * time.Duration(timeout)}
	var gnURL string
	if sandbox {
		gnURL = "https://sandbox.gerencianet.com.br/v1"
	} else {
		gnURL = "https://api.gerencianet.com.br/v1"
	}
	return &requester{*auth, gnURL, timeout, "", time.Time{}, httpClient}
}

func RequesterOf(clientID string, clientSecret string, sandbox bool, timeout int) *requester {
	auth := newAuth(clientID, clientSecret, sandbox, timeout)
	httpClient := &http.Client{Timeout: time.Second * time.Duration(timeout)}
	var gnURL string
	if sandbox {
		gnURL = "https://apis.gerencianet.com.br/open-finance"
	} else {
		gnURL = "https://apis.gerencianet.com.br/open-finance"
	}
	return &requester{*auth, gnURL, timeout, "", time.Time{}, httpClient}
